...
public class BardClass extends PvpClass implements Listener {
	...
	public BardClass(HCFaction plugin) {
	...
	public boolean onEquip(Player player) {
		...
		if (BardClass.this.plugin.getTimerManager().pvpProtectionTimer.getRemaining(player.getUniqueId()) > 0L) {
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		...
		if ((action == Action.RIGHT_CLICK_AIR) || ((!event.isCancelled()) && (action == Action.RIGHT_CLICK_BLOCK))) {
			...
			if (HCFaction.getPlugin().getSotwTimer().getSotwRunnable() != null
					&& !HCFaction.getPlugin().getSotwTimer().getSotwRunnable().isSotwEnabled(player)) {
				player.sendMessage(CC.translate("&cEffects cant be used in Sotw Timer!."));
				return;
			}
			if (bardData != null) {
				...
				if ((bardEffect != null)
					...
					PlayerFaction playerFaction = this.plugin.getFactionManager()
							.getPlayerFaction(player.getUniqueId());
	...
	private boolean canUseBardEffect(Player player, BardData bardData, BardEffect bardEffect, boolean sendFeedback) {
		...
		if (remaining > 0L) {
			errorFeedback = ChatColor.RED + "You still have a cooldown on this " + ChatColor.GREEN + ChatColor.BOLD
					+ "Bard" + ChatColor.RED + " buff for another " + HCFaction.getRemaining(remaining, true, false)
					+ ChatColor.RED + '.';
		}

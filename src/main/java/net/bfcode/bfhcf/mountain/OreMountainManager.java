...
public class OreMountainManager implements Runnable {
    ...
    private static long DEFAULTTIME = TimeUnit.HOURS.toMillis(1);
    public Set<Integer> ALLOWED = new HashSet<>(Arrays.asList(Material.COAL_ORE.getId(), Material.DIAMOND_ORE.getId(), Material.EMERALD_ORE.getId(), Material.GLOWING_REDSTONE_ORE.getId(), Material.GOLD_ORE.getId(), Material.IRON_ORE.getId(), Material.LAPIS_ORE.getId(), Material.REDSTONE_ORE.getId()));
    private HCFaction hcf;
    private File schematicfile = new File("plugins/WorldEdit/schematics/oremountain.schematic");
    ...
    public OreMountainManager(HCFaction hcf) {
        this.hcf = hcf;
